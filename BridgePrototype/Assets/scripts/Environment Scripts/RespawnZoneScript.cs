...
public class RespawnZoneScript : MonoBehaviour {
	...
	{
		if (hasP1 && hasP2)
		if (cam.GetComponent<CameraScript> ().player1.GetComponent<PlayerScript> ().respawnPosition == P1S) {
			gameObject.GetComponentInChildren<MeshRenderer>().material = material[1];
		}
		else
			gameObject.GetComponentInChildren<MeshRenderer>().material  = material[0];
	}
