...
public class PlayerScript : MonoBehaviour {
	...
	void Update()
	{
		...
		if (meldObject) { //&& meldObject.tag == "SingleControlRock")
			transform.position = new Vector3 (meldObject.transform.position.x,
			                                 meldObject.transform.position.y,
			                                 meldObject.transform.position.z);
       ...
       // DisplayPrompts ();
        ...
        if (melding) {
			...
			if(!melding)
			if(Mathf.Sqrt(Mathf.Pow(Input.GetAxis (myLeftStick + "X"),2) + Mathf.Pow(Input.GetAxis (myLeftStick + "Y"), 2)) > .9f)
			{
				if(speed < maxSpeed)
					speed += .05f;
			}
			...
			if (Input.GetButtonDown (myXButton) && airborne == false && !melding) {
	...
	void DisplayPrompts()
	{
		...
		if(rig.velocity.magnitude > maxSpeed * 5)
		{
	...
	void OnCollisionEnter(Collision col)
	{
		if (col.gameObject.tag == "Projectile") {
			rig.AddForce ((col.gameObject.transform.position - transform.position).normalized * 2f);
	...
	void OnTriggerEnter(Collider col)
	{
		if (col.gameObject.tag == "Collectable") {
			col.gameObject.GetComponentInParent<Collectable>().Activate();
